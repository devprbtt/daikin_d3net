...
def _load_rtu_module():
    """Load d3net_rtu_tool from import path or sibling file.

    This makes the GUI easier to run on Windows when users copy scripts
    to a folder that is not installed as a package.
    """

    try:
        import d3net_rtu_tool as rtu_module  # type: ignore

        return rtu_module
    except ModuleNotFoundError:
        script_dir = Path(__file__).resolve().parent
        tool_path = script_dir / "d3net_rtu_tool.py"
        if not tool_path.exists():
            raise ModuleNotFoundError(
                "Could not find d3net_rtu_tool.py. Keep d3net_rtu_gui.py and "
                "d3net_rtu_tool.py in the same folder, or install as a package."
            )

        spec = importlib.util.spec_from_file_location("d3net_rtu_tool", tool_path)
        if spec is None or spec.loader is None:
            raise ModuleNotFoundError(f"Unable to load module from {tool_path}")

        rtu_module = importlib.util.module_from_spec(spec)
        sys.modules["d3net_rtu_tool"] = rtu_module
        spec.loader.exec_module(rtu_module)
        return rtu_module


rtu = _load_rtu_module()
